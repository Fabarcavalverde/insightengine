...
class pca_reducer:
    ...
    Uso:
        pca = PCAReducer(df, n_components=2, n_clusters=3)
        resultado = pca.fit()
    """
    ...
    def __init__(
        ...
        """
        Objetivo:
            Inicializar el reductor con los datos ya estandarizados internamente.
    ...
    def fit(self) -> pd.DataFrame:
        """
        Objetivo:
            Aplicar ACP y K-Medias sobre los datos estandarizados.
        ...
        """
        # reducción
        acp = PCA(n_components=self.n_components).fit_transform(self.datos_std)
        cols = [f"dim{i+1}" for i in range(self.n_components)]
        self.df_resultado = pd.DataFrame(acp, columns=cols, index=self.datos_std.index)
